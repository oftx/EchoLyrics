...
// IPC Handlers
// PiP Window Control
electron_1.ipcMain.handle('pip:open', () => {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
        ...
        pipWindow.loadFile(path_1.default.join(__dirname, '../dist/index.html'), {
            hash: 'pip'
        });
    }
    pipWindow.on('closed', () => {
...
electron_1.ipcMain.handle('pip:close', () => {
    if (pipWindow && !pipWindow.isDestroyed()) {
        pipWindow.close();
        return { success: true };
    }
...
electron_1.ipcMain.handle('pip:syncState', (_event, state) => {
    if (pipWindow && !pipWindow.isDestroyed()) {
...
const qq_decrypt_1 = require("./services/qq-decrypt");
electron_1.ipcMain.handle('net:request', async (_event, config) => {
    ...
    try {
        ...
        if (url.includes('/api/qq-decrypt')) {
            ...
            return await (0, qq_decrypt_1.fetchQQWithDecrypt)(urlObj.pathname, params);
        }
